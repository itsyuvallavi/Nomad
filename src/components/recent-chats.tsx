        ...
        <div className="w-full mt-12 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {recentChatData.map((chat, index) => (
                    <Card key={index} className="p-3 flex flex-col justify-between cursor-pointer hover:bg-secondary/50 transition-colors animate-in fade-in zoom-in-95" style={{ animationDelay: `${index * 150}ms`}}>
                        <div>
                            <div className="mb-3">{chat.icon}</div>
                            <h3 className="font-semibold text-foreground text-sm">{chat.title}</h3>
                        </div>
                        <p className="text-xs text-muted-foreground mt-2">{chat.subtitle}</p>
                    </Card>
                ))}
            </div>
        </div>
