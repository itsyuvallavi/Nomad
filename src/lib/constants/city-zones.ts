/**
 * City Zones Configuration
 * Defines logical zones/neighborhoods for major cities to help with route optimization
 * and coherent itinerary planning
 */

export interface CityZone {
  name: string;
  areas: string[];
  attractions: string[];
  restaurants: string[];
  coordinates: { lat: number; lng: number };
  description?: string;
}

export interface CityConfig {
  zones: CityZone[];
  transitInfo?: {
    metro?: boolean;
    bus?: boolean;
    tram?: boolean;
    bike?: boolean;
  };
  walkingCity?: boolean;
}

export const CITY_ZONES: Record<string, CityConfig> = {
  Paris: {
    zones: [
      {
        name: "Eiffel Tower & Trocadéro",
        areas: ["7th arrondissement", "16th arrondissement"],
        attractions: [
          "Eiffel Tower",
          "Trocadéro Gardens",
          "Musée Rodin",
          "Invalides",
          "Champ de Mars"
        ],
        restaurants: [
          "Jules Verne",
          "Café de l'Homme",
          "Girafe",
          "Les Ombres"
        ],
        coordinates: { lat: 48.8584, lng: 2.2945 },
        description: "Iconic tower area with museums and gardens"
      },
      {
        name: "Latin Quarter & Saint-Germain",
        areas: ["5th arrondissement", "6th arrondissement"],
        attractions: [
          "Panthéon",
          "Luxembourg Gardens",
          "Shakespeare and Company",
          "Notre-Dame Cathedral",
          "Sainte-Chapelle"
        ],
        restaurants: [
          "Café de Flore",
          "Les Deux Magots",
          "Le Comptoir du Relais",
          "L'Ami Jean"
        ],
        coordinates: { lat: 48.8462, lng: 2.3454 },
        description: "Historic student quarter with cafés and bookshops"
      },
      {
        name: "Marais",
        areas: ["3rd arrondissement", "4th arrondissement"],
        attractions: [
          "Place des Vosges",
          "Jewish Quarter",
          "Centre Pompidou",
          "Musée Carnavalet",
          "Hôtel de Ville"
        ],
        restaurants: [
          "L'As du Fallafel",
          "Breizh Café",
          "Chez Janou",
          "Au Bourguignon du Marais"
        ],
        coordinates: { lat: 48.8566, lng: 2.3617 },
        description: "Trendy historic district with Jewish heritage"
      },
      {
        name: "Louvre & Opéra",
        areas: ["1st arrondissement", "2nd arrondissement", "9th arrondissement"],
        attractions: [
          "Louvre Museum",
          "Palais Royal",
          "Opéra Garnier",
          "Tuileries Garden",
          "Place Vendôme"
        ],
        restaurants: [
          "Le Meurice",
          "Angelina",
          "Kitsuné",
          "Le Grand Véfour"
        ],
        coordinates: { lat: 48.8606, lng: 2.3376 },
        description: "Royal palaces and world-class museums"
      },
      {
        name: "Champs-Élysées & Arc de Triomphe",
        areas: ["8th arrondissement", "17th arrondissement"],
        attractions: [
          "Arc de Triomphe",
          "Champs-Élysées",
          "Grand Palais",
          "Petit Palais",
          "Parc Monceau"
        ],
        restaurants: [
          "L'Avenue",
          "Fouquet's",
          "Le Cinq",
          "Ladurée"
        ],
        coordinates: { lat: 48.8738, lng: 2.2950 },
        description: "Luxury shopping and grand boulevards"
      },
      {
        name: "Montmartre",
        areas: ["18th arrondissement"],
        attractions: [
          "Sacré-Cœur",
          "Place du Tertre",
          "Moulin Rouge",
          "Montmartre Cemetery",
          "Musée de Montmartre"
        ],
        restaurants: [
          "La Maison Rose",
          "Le Consulat",
          "Chez la Mère Catherine",
          "Le Moulin de la Galette"
        ],
        coordinates: { lat: 48.8867, lng: 2.3431 },
        description: "Bohemian hilltop village with artists and cabarets"
      }
    ],
    transitInfo: {
      metro: true,
      bus: true,
      bike: true
    },
    walkingCity: true
  },

  London: {
    zones: [
      {
        name: "Westminster & South Bank",
        areas: ["Westminster", "Southbank", "Waterloo"],
        attractions: [
          "Big Ben",
          "Westminster Abbey",
          "London Eye",
          "Tate Modern",
          "Shakespeare's Globe",
          "Borough Market"
        ],
        restaurants: [
          "Dishoom Covent Garden",
          "The Ivy",
          "Aqua Shard",
          "OXO Tower Restaurant"
        ],
        coordinates: { lat: 51.5007, lng: -0.1246 },
        description: "Political center and riverside culture"
      },
      {
        name: "Covent Garden & Soho",
        areas: ["Covent Garden", "Soho", "Chinatown"],
        attractions: [
          "British Museum",
          "National Gallery",
          "Covent Garden Market",
          "Leicester Square",
          "Chinatown"
        ],
        restaurants: [
          "The Ivy Market Grill",
          "Yauatcha",
          "Barrafina",
          "Sketch"
        ],
        coordinates: { lat: 51.5146, lng: -0.1243 },
        description: "Theatre district and cultural heart"
      },
      {
        name: "The City & Tower Bridge",
        areas: ["City of London", "Tower Hill", "Shoreditch"],
        attractions: [
          "Tower of London",
          "Tower Bridge",
          "St. Paul's Cathedral",
          "Sky Garden",
          "Leadenhall Market"
        ],
        restaurants: [
          "Duck & Waffle",
          "Hawksmoor",
          "The Breakfast Club",
          "Dishoom Shoreditch"
        ],
        coordinates: { lat: 51.5074, lng: -0.0759 },
        description: "Historic fortress and modern skyscrapers"
      },
      {
        name: "Kensington & Chelsea",
        areas: ["South Kensington", "Chelsea", "Knightsbridge"],
        attractions: [
          "Natural History Museum",
          "Victoria & Albert Museum",
          "Hyde Park",
          "Harrods",
          "Royal Albert Hall"
        ],
        restaurants: [
          "Dinner by Heston",
          "The Ledbury",
          "Zuma",
          "Aubaine"
        ],
        coordinates: { lat: 51.4969, lng: -0.1748 },
        description: "Royal parks and world-class museums"
      },
      {
        name: "Camden & Regent's Park",
        areas: ["Camden Town", "Regent's Park", "King's Cross"],
        attractions: [
          "Camden Market",
          "Regent's Park",
          "London Zoo",
          "British Library",
          "Primrose Hill"
        ],
        restaurants: [
          "The Cheese Bar",
          "Dishoom King's Cross",
          "Chin Chin Labs",
          "Proud Camden"
        ],
        coordinates: { lat: 51.5391, lng: -0.1426 },
        description: "Alternative culture and royal parks"
      },
      {
        name: "Notting Hill & Paddington",
        areas: ["Notting Hill", "Paddington", "Bayswater"],
        attractions: [
          "Portobello Road Market",
          "Notting Hill",
          "Hyde Park",
          "Little Venice",
          "Paddington Station"
        ],
        restaurants: [
          "Dishoom Kensington",
          "The Ledbury",
          "Granger & Co",
          "Electric Diner"
        ],
        coordinates: { lat: 51.5142, lng: -0.2054 },
        description: "Colorful houses and antique markets"
      }
    ],
    transitInfo: {
      metro: true,
      bus: true,
      bike: true
    },
    walkingCity: true
  },

  Tokyo: {
    zones: [
      {
        name: "Shibuya & Harajuku",
        areas: ["Shibuya", "Harajuku", "Omotesando"],
        attractions: [
          "Shibuya Crossing",
          "Meiji Shrine",
          "Takeshita Street",
          "Omotesando Hills",
          "Yoyogi Park"
        ],
        restaurants: [
          "Ichiran Ramen",
          "Gonpachi",
          "Bills Omotesando",
          "Kawaii Monster Cafe"
        ],
        coordinates: { lat: 35.6580, lng: 139.7016 },
        description: "Youth culture and fashion capital"
      },
      {
        name: "Shinjuku",
        areas: ["Shinjuku", "Kabukicho"],
        attractions: [
          "Shinjuku Gyoen",
          "Tokyo Metropolitan Building",
          "Golden Gai",
          "Kabukicho",
          "Robot Restaurant"
        ],
        restaurants: [
          "Nabezo",
          "Kozue",
          "New York Grill",
          "Omoide Yokocho"
        ],
        coordinates: { lat: 35.6938, lng: 139.7034 },
        description: "Neon lights and entertainment district"
      },
      {
        name: "Asakusa & Ueno",
        areas: ["Asakusa", "Ueno", "Yanaka"],
        attractions: [
          "Senso-ji Temple",
          "Tokyo Skytree",
          "Ueno Park",
          "Tokyo National Museum",
          "Yanaka Cemetery"
        ],
        restaurants: [
          "Asakusa Imahan",
          "Daikokuya",
          "Sometaro",
          "Kamiya Bar"
        ],
        coordinates: { lat: 35.7148, lng: 139.7967 },
        description: "Traditional temples and old Tokyo"
      },
      {
        name: "Ginza & Tsukiji",
        areas: ["Ginza", "Tsukiji", "Yurakucho"],
        attractions: [
          "Tsukiji Outer Market",
          "Ginza Shopping",
          "Kabuki-za Theatre",
          "Imperial Palace East Gardens",
          "GINZA SIX"
        ],
        restaurants: [
          "Sukiyabashi Jiro",
          "Kyubey",
          "Ginza Kojyu",
          "Tsukiji Sushidai"
        ],
        coordinates: { lat: 35.6719, lng: 139.7670 },
        description: "Luxury shopping and sushi paradise"
      },
      {
        name: "Roppongi & Akasaka",
        areas: ["Roppongi", "Akasaka", "Azabu"],
        attractions: [
          "Tokyo Tower",
          "Roppongi Hills",
          "Mori Art Museum",
          "National Art Center",
          "Tokyo Midtown"
        ],
        restaurants: [
          "Nobu Tokyo",
          "L'Atelier de Joel Robuchon",
          "Gonpachi Roppongi",
          "Two Rooms"
        ],
        coordinates: { lat: 35.6607, lng: 139.7295 },
        description: "Nightlife and international dining"
      }
    ],
    transitInfo: {
      metro: true,
      bus: true,
      bike: false
    },
    walkingCity: false
  },

  "New York": {
    zones: [
      {
        name: "Midtown Manhattan",
        areas: ["Times Square", "Hell's Kitchen", "Theater District"],
        attractions: [
          "Times Square",
          "Empire State Building",
          "Rockefeller Center",
          "Broadway Theaters",
          "MoMA"
        ],
        restaurants: [
          "Le Bernardin",
          "Eleven Madison Park",
          "Joe's Pizza",
          "Katz's Delicatessen"
        ],
        coordinates: { lat: 40.7580, lng: -73.9855 },
        description: "Bright lights and Broadway shows"
      },
      {
        name: "Central Park & Upper East Side",
        areas: ["Central Park", "Upper East Side", "Museum Mile"],
        attractions: [
          "Central Park",
          "Metropolitan Museum",
          "Guggenheim Museum",
          "The Frick Collection",
          "Museum Mile"
        ],
        restaurants: [
          "The Loeb Boathouse",
          "Daniel",
          "Cafe Boulud",
          "Serafina"
        ],
        coordinates: { lat: 40.7829, lng: -73.9654 },
        description: "Green oasis and world-class museums"
      },
      {
        name: "Downtown & Financial District",
        areas: ["Financial District", "Battery Park", "Tribeca"],
        attractions: [
          "One World Trade Center",
          "9/11 Memorial",
          "Statue of Liberty",
          "Wall Street",
          "Brooklyn Bridge"
        ],
        restaurants: [
          "Delmonico's",
          "The Dead Rabbit",
          "Stone Street Tavern",
          "Nobu Downtown"
        ],
        coordinates: { lat: 40.7074, lng: -74.0113 },
        description: "Historic harbor and financial center"
      },
      {
        name: "Greenwich Village & SoHo",
        areas: ["Greenwich Village", "SoHo", "NoHo"],
        attractions: [
          "Washington Square Park",
          "High Line",
          "Whitney Museum",
          "Comedy Cellar",
          "SoHo Shopping"
        ],
        restaurants: [
          "Blue Hill",
          "Carbone",
          "Joe's Pizza",
          "Mamoun's Falafel"
        ],
        coordinates: { lat: 40.7308, lng: -73.9973 },
        description: "Bohemian charm and trendy shopping"
      },
      {
        name: "Brooklyn",
        areas: ["DUMBO", "Williamsburg", "Brooklyn Heights"],
        attractions: [
          "Brooklyn Bridge Park",
          "DUMBO",
          "Brooklyn Museum",
          "Prospect Park",
          "Coney Island"
        ],
        restaurants: [
          "Peter Luger",
          "Grimaldi's",
          "Junior's",
          "Roberta's Pizza"
        ],
        coordinates: { lat: 40.6782, lng: -73.9442 },
        description: "Hip neighborhoods across the river"
      }
    ],
    transitInfo: {
      metro: true,
      bus: true,
      bike: true
    },
    walkingCity: true
  },

  Barcelona: {
    zones: [
      {
        name: "Gothic Quarter & Las Ramblas",
        areas: ["Barri Gòtic", "El Raval", "Las Ramblas"],
        attractions: [
          "Barcelona Cathedral",
          "Las Ramblas",
          "Plaça Reial",
          "La Boqueria Market",
          "MACBA"
        ],
        restaurants: [
          "Els Quatre Gats",
          "Can Culleretes",
          "Bar del Pla",
          "Cervecería Catalana"
        ],
        coordinates: { lat: 41.3825, lng: 2.1769 },
        description: "Medieval streets and historic heart"
      },
      {
        name: "Eixample & Sagrada Familia",
        areas: ["Eixample", "Gràcia"],
        attractions: [
          "Sagrada Familia",
          "Casa Batlló",
          "Casa Milà",
          "Park Güell",
          "Passeig de Gràcia"
        ],
        restaurants: [
          "Tickets Bar",
          "Disfrutar",
          "Casa Calvet",
          "Cervecería Catalana"
        ],
        coordinates: { lat: 41.4036, lng: 2.1744 },
        description: "Gaudí's masterpieces and modernist architecture"
      },
      {
        name: "Barceloneta & Waterfront",
        areas: ["Barceloneta", "Port Vell", "Vila Olímpica"],
        attractions: [
          "Barceloneta Beach",
          "Port Vell",
          "Aquarium Barcelona",
          "Cable Car",
          "W Hotel Barcelona"
        ],
        restaurants: [
          "Can Majó",
          "La Cova Fumada",
          "Salamanca",
          "Els Pescadors"
        ],
        coordinates: { lat: 41.3774, lng: 2.1896 },
        description: "Beach life and seafood restaurants"
      },
      {
        name: "Montjuïc",
        areas: ["Montjuïc", "Poble Sec"],
        attractions: [
          "Montjuïc Castle",
          "Magic Fountain",
          "MNAC",
          "Fundació Joan Miró",
          "Olympic Stadium"
        ],
        restaurants: [
          "Tickets",
          "Quimet & Quimet",
          "Martinez",
          "La Caseta del Migdia"
        ],
        coordinates: { lat: 41.3633, lng: 2.1585 },
        description: "Hilltop parks and Olympic legacy"
      }
    ],
    transitInfo: {
      metro: true,
      bus: true,
      tram: true,
      bike: true
    },
    walkingCity: true
  }
};

/**
 * Get zone recommendations for a specific city and day
 */
export function getZoneRecommendations(city: string, day: number): CityZone[] | null {
  const cityConfig = CITY_ZONES[city];
  if (!cityConfig) return null;

  const zones = cityConfig.zones;
  if (zones.length === 0) return null;

  // Recommend 1-2 zones per day for efficient exploration
  const zonesPerDay = Math.min(2, Math.ceil(zones.length / 3));
  const startIndex = (day - 1) * zonesPerDay;
  const endIndex = Math.min(startIndex + zonesPerDay, zones.length);

  return zones.slice(startIndex, endIndex);
}

/**
 * Find the zone for a specific venue
 */
export function findZoneForVenue(city: string, venueName: string): CityZone | null {
  const cityConfig = CITY_ZONES[city];
  if (!cityConfig) return null;

  for (const zone of cityConfig.zones) {
    // Check attractions
    if (zone.attractions.some(attraction =>
      attraction.toLowerCase().includes(venueName.toLowerCase()) ||
      venueName.toLowerCase().includes(attraction.toLowerCase())
    )) {
      return zone;
    }

    // Check restaurants
    if (zone.restaurants.some(restaurant =>
      restaurant.toLowerCase().includes(venueName.toLowerCase()) ||
      venueName.toLowerCase().includes(restaurant.toLowerCase())
    )) {
      return zone;
    }
  }

  return null;
}

/**
 * Get nearby zones (adjacent or close zones)
 */
export function getNearbyZones(city: string, currentZone: CityZone): CityZone[] {
  const cityConfig = CITY_ZONES[city];
  if (!cityConfig) return [];

  const MAX_DISTANCE_KM = 3; // Consider zones within 3km as nearby

  return cityConfig.zones.filter(zone => {
    if (zone.name === currentZone.name) return false;

    const distance = calculateZoneDistance(currentZone, zone);
    return distance <= MAX_DISTANCE_KM;
  });
}

/**
 * Calculate approximate distance between zone centers
 */
function calculateZoneDistance(zone1: CityZone, zone2: CityZone): number {
  const R = 6371; // Earth's radius in km
  const dLat = toRad(zone2.coordinates.lat - zone1.coordinates.lat);
  const dLng = toRad(zone2.coordinates.lng - zone1.coordinates.lng);

  const a =
    Math.sin(dLat / 2) * Math.sin(dLat / 2) +
    Math.cos(toRad(zone1.coordinates.lat)) *
    Math.cos(toRad(zone2.coordinates.lat)) *
    Math.sin(dLng / 2) * Math.sin(dLng / 2);

  const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
  return R * c;
}

function toRad(degrees: number): number {
  return degrees * (Math.PI / 180);
}

/**
 * Get transit recommendation for a city
 */
export function getTransitRecommendation(city: string): string {
  const cityConfig = CITY_ZONES[city];
  if (!cityConfig) return "Check local transit options";

  const transit = cityConfig.transitInfo;
  if (!transit) return "Walking and public transit recommended";

  const options = [];
  if (transit.metro) options.push("Metro");
  if (transit.bus) options.push("Bus");
  if (transit.tram) options.push("Tram");
  if (transit.bike) options.push("Bike sharing");

  if (cityConfig.walkingCity) {
    return `Walking-friendly city. Also available: ${options.join(", ")}`;
  } else {
    return `Public transit recommended: ${options.join(", ")}`;
  }
}

// Export utility functions
export const cityZones = {
  getZoneRecommendations,
  findZoneForVenue,
  getNearbyZones,
  getTransitRecommendation,
};

export default cityZones;