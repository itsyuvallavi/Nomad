...
export class DestinationParser {
  ...
  extractSingleDestination(message: string): string | null {
    const destinationPatterns = [
      ...
      // Capture only the city name, stop before date-related keywords
      /(?:\d+\s+(?:day|night)s?\s+(?:trip\s+)?(?:to|in))\s+([a-z]+(?:\s+[a-z]+)?)(?=\s+(?:starting|beginning|ending|from|on|in|for|tomorrow|today|next|this|week|month|year)|[,.]|$)/i,
      // "trip to London starting tomorrow" OR "trip to London for tomorrow" -> extract "London" only
      /(?:trip to|visit|going to|travel to|fly to|head to|explore|tour|plan.*to)\s+([a-z]+(?:\s+[a-z]+)?)(?=\s+(?:for|from|starting|beginning|ending|on|in|next|this|tomorrow|today|week|month|year|\d+)|[,.]|$)/i,
      // "3 days in London starting tomorrow" -> extract "London" (not "London Starting")
      /(\d+)\s+(?:days?|nights?)\s+(?:in|to)\s+([a-z]+(?:\s+[a-z]+)?)(?=\s+(?:starting|beginning|ending|from|on|for|tomorrow|today|next|this|week|month|year)|$)/i,
      // "London for 3 days" -> extract "London" (but don't capture "for tomorrow")
    ...
    for (const pattern of destinationPatterns) {
      ...
      if (match) {
        let destination = (match[1] || match[2] || '').trim();

        // Extra safety: Remove any trailing date-related words that snuck through
        destination = destination.replace(/\s+(starting|beginning|ending|tomorrow|today|next|this|week|month|year)$/i, '');
