...
/**
 * Get city center coordinates from the first zone (highest priority)
 * Returns null if city not found in database
 */
export function getCityCoordinates(city: string): { lat: number; lng: number } | null {
  const zones = getCityZones(city);
  if (zones.length === 0) {
    logger.warn('CityZones', `No zones found for city: ${city}`);
    return null;
  }

  // Return coordinates from highest priority zone (usually city center)
  const centerZone = sortZonesByPriority(zones)[0];
  logger.info('CityZones', `City coordinates for ${city}`, centerZone.coordinates);
  return centerZone.coordinates;
}