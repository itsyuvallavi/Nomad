...
import { getCityZones, getCityCoordinates } from '../data/city-zones';
...
export class ItineraryEnricher {
  ...
  private async enrichWithHERE(
    ...
    try {
      ...
      // Prepare search queries with location context
      const searchQueries = activitiesToEnrich.map(item => {
        ...
        return {
          query: finalQuery,
          location: cityCoords || undefined // Add city coordinates for HERE API
        };
      });
